...
public class List {
	...
	static void remove(ArrayList<Integer> ar, int ele) {
		for(int i = 0; i < ar.size(); i++) {
			if(ar.get(i) == ele) {
				ar.remove(i);
				System.out.println("Number is removed Succesfully");
				return;
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("List is created and intialized with 10 elements");
		boolean flag = true;
		while(flag) {
			...
			if(op == 1) {
				...
				int ele = sc.nextInt();
				ar.add(ele);
				System.out.println("Number is added in the list");
			} else if(op == 3) {
